...
// ==========================================================================
// REAL-TIME EDITOR SLIDER LIVE MOCKUP PREVIEW (FITUR PENYESUAIAN)
// ==========================================================================
function initAestheticSliders() {
  const rangeBright = document.getElementById('rangeBright');
  const rangeContrast = document.getElementById('rangeContrast');
  const rangeSat = document.getElementById('rangeSat'); // Ambil slider baru

  // Gabungkan semua slider ke dalam satu fungsi pantau
  [rangeBright, rangeContrast, rangeSat].forEach(slider => {
    if (slider) {
      // Menggunakan event 'input' agar perubahan langsung terasa pas digeser (real-time)
      slider.addEventListener('input', () => {
        const bVal = rangeBright ? rangeBright.value : 100;
        const cVal = rangeContrast ? rangeContrast.value : 100;
        const sVal = rangeSat ? rangeSat.value : 100;
        
        // Terapkan efek filter CSS langsung ke seluruh gambar mockup di strip preview
...
// ==========================================================================
// BIND ON LOAD (PENGUNCI UTAMA SAAT HALAMAN DIBUKA)
// ==========================================================================
window.addEventListener('DOMContentLoaded', () => { 
  initPhotostrip(); 
  bindFrameSelection();
  ...
  // JALANKAN FUNGSI SLIDER DISINI BIAR AKTIF DAN BERFUNGSI
  initAestheticSliders(); 
  ...
  if (resetBtn) {
    resetBtn.addEventListener('click', () => {
      ...
      // Reset ulang nilai slider ke posisi normal (100) saat mulai sesi baru
      if (document.getElementById('rangeBright')) document.getElementById('rangeBright').value = 100;
      if (document.getElementById('rangeContrast')) document.getElementById('rangeContrast').value = 100;
      if (document.getElementById('rangeSat')) document.getElementById('rangeSat').value = 100;
    });
  }
});